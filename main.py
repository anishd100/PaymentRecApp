...
def download_report(contractor_id):
    ...
    try:
        ...
        # 6. Invoice Payments - ONLY fetch Total_amount and utr now
        cursor.execute("""
        ...
        # 7. GST Release Payments (ONLY Total_amount and utr)
        cursor.execute("""
            SELECT p.pmc_no, p.invoice_no, p.Total_amount, p.utr
            FROM payment p
        ...
        for inv in invoices:
            ...
            if (pmc_no, invoice_no) in gst_rel_map:
                ...
                # Extra Payment Row (payments with pmc_no but no invoice_no)
                cursor.execute("""
                    SELECT Total_amount, utr
                    FROM payment
                    WHERE pmc_no = %s AND (invoice_no IS NULL OR invoice_no = '')
                      AND Total_amount != 0
                    LIMIT 1
                """, (pmc_no,))
        ...
        for row in sheet.iter_rows(min_row=13, max_row=sheet.max_row, values_only=True):
            try:
                ...
                total_sd_amount += float(row[12] or 0)
                total_on_commission += float(row[13] or 0)
                total_final_amount += float(row[-3] or 0)  # Final Amount column index shifted
                total_total_amount += float(row[-2] or 0)  # Total Paid
        ...
        sheet.append([])
        sheet.append(["Contractor Name", contInfo["Contractor_Name"]])
        sheet.append(["Date", datetime.today().strftime('%A, %Y-%m-%d')])
        sheet.append(["Description", "Amount"])
        sheet.append(["Advance/Surplus", str(total_final_amount - total_total_amount)])
        sheet.append(["Total Hold Amount", str(Decimal(total_hold_amount))])
        sheet.append(["Amount With TDS", str(total_tds_amount)])
